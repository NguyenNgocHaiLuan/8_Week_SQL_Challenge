...
-- Table thứ 3 customer_transactions--
...
SELECT
	a.region_id,
	b.region_name,
	COUNT(DISTINCT node_id) as count_code
...
WITH Day_in_node as (
	SELECT
		customer_id,
		node_id,
		-- => Lý do dùng SUM vì trong dữ liệu 1 node có thể chứa 2 dòng, nếu dùng AVG luôn thì sẽ (dòng 1 + dòng 2)/2 nên sẽ bị sai
		-- Ví dụ như ở node 1 có 1 ông khách hàng có tận 2 dòng, cột start_date bắt đầu từ ngày 15/10 và end_date kết thúc lúc 20/10
		-- dòng thứ 2 thì cột start_date bắt đầu 21/10 và kết thúc lúc 29/10
		-- => Chúng ta mường tượng ra là dùng AVG thì trước thì nó sẽ là (15->20) là 5 ngày và (21->29) là 9 ngày 
		-- => ( 9 + 5 ) / 2 = 7 là sai
		-- => Cho nên chúng ta cần phải sử dụng sum trong CTE để cộng DAY lại rồi mới sử dụng AVG
		SUM(DATEDIFF(DAY, start_date, end_date)) as day_customer 
	FROM customer_nodes
	WHERE end_date != '9999-12-31'
	GROUP BY customer_id, node_id
)
SELECT
	ROUND(AVG(day_customer),0) as avg_day_customer
FROM Day_in_node
...
SELECT
	...
	PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY duration) OVER (PARTITION BY region_name) AS median,
	PERCENTILE_CONT(0.8) WITHIN GROUP (ORDER BY duration) OVER (PARTITION BY region_name) AS percentile_80,
	PERCENTILE_CONT(0.95) WITHIN GROUP (ORDER BY duration) OVER (PARTITION BY region_name) AS percentile_95
FROM day_region
...
SELECT 
	...
	WHERE rn IN (ROUND(M.max_rn /2,0), ROUND(M.max_rn * 0.8,0), ROUND(M.max_rn * 0.95,0)
);
----B. Customer Transactions----
-- 1. What is the unique count and total amount for each transaction type?
SELECT
	txn_type,
	SUM(txn_amount) AS customer_count,
...
SELECT
	ROUND(AVG(avg_amount),2) as avg_total_deposit,
	ROUND(AVG(transactions_count),0) as avg_transactions
FROM cte
...
SELECT
	...
	month_end_date,
	monthly_change,
	SUM(monthly_change) OVER(PARTITION BY customer_id ORDER BY month_end_date ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS closing_balance
...
WITH CTE AS (
	...
	FROM customer_transactions
	GROUP BY DATETRUNC(month,txn_date), txn_date, customer_id
),BALANCES AS (
	SELECT 
	*
...
closing_balances AS (
    SELECT 
        customer_id,
        month_end_date,
        SUM(monthly_amount) OVER(PARTITION BY customer_id ORDER BY month_end_date ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS closing_balance
    FROM monthly_balances
),
first_last_balance AS (
    SELECT DISTINCT
        customer_id,
        FIRST_VALUE(closing_balance) OVER(PARTITION BY customer_id ORDER BY month_end_date ASC) AS first_balance,
        LAST_VALUE(closing_balance) OVER(PARTITION BY customer_id ORDER BY month_end_date ASC ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS last_balance
    FROM closing_balances
)
SELECT 
    CAST(COUNT(*) * 100.0 / (SELECT COUNT(*) FROM first_last_balance) AS FLOAT) AS pct_customers
FROM first_last_balance
WHERE first_balance > 0 AND last_balance > (first_balance * 1.05);
...
monthly_allocation AS (
    SELECT 
        ...
        MAX(
			CASE 
				WHEN rn_end = 1 THEN running_balance 
			END) AS option_1_end_date,
		--option 2
        AVG(running_balance) AS option_2_average,  
		--option 3
        MAX(running_balance) AS option_3_realtime
    FROM (
        SELECT 
            *,
            ROW_NUMBER() OVER(PARTITION BY customer_id, month_id ORDER BY txn_date DESC) as rn_end
...
SELECT 
    ...
    SUM(
		CASE 
			WHEN option_1_end_date > 0 THEN option_1_end_date 
			ELSE 0 
		END) AS allocation_option_1,
    SUM(
		CASE 
			WHEN option_2_average > 0 THEN option_2_average 
			ELSE 0 
		END) AS allocation_option_2,
    SUM(
		CASE 
			WHEN option_3_realtime > 0 THEN option_3_realtime 
			ELSE 0 
		END) AS allocation_option_3
...
--Learn--
/*
PERCENTILE_CONT( tỷ lệ ) WITHIN GROUP ( ORDER BY ...) OVER(PARTITION BY ...)
EOMONTH(txn_date) chỉ lấy cuối tháng
DATETRUNC
DATEADD
SUM(monthly_change) OVER(PARTITION BY customer_id ORDER BY month_end_date ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS closing_balance
*/